        ...
        $rootNode
            ->children()
                ->arrayNode('secrets')
                    ->canBeEnabled()
                    ->children()
                        ->scalarNode('encrypted_secrets_dir')->defaultValue('%kernel.project_dir%/config/secrets/%kernel.environment%/encrypted')->cannotBeEmpty()->end()
                        ->scalarNode('encryption_key')->end()
                        ->scalarNode('encryption_public_key')->end()
                        ->scalarNode('encryption_private_key')->end()
                    ->end()
                    ->validate()
                        ->ifTrue(function ($v) { return !empty($v['encryption_key']) && !empty($v['encryption_private_key']); })
                        ->thenInvalid('"encryption_key" and "encryption_private_key" cannot be used together.')
                    ->end()
                    ->validate()
                        ->ifTrue(function ($v) { return !empty($v['encryption_private_key']) && empty($v['encryption_public_key']); })
                        ->thenInvalid('when using "encryption_private_key", "encryption_public_key" have to be defined too.')
                    ->end()
                    ->validate()
                        ->ifTrue(function ($v) { return empty($v['encryption_key']) && empty($v['encryption_private_key']); })
                        ->thenInvalid('either "encryption_key" or "encryption_private_key" have to be defiened.')
                    ->end()
                ->end()
            ->end()
        ;
